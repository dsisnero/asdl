...
module Asdl
  class TypeDefVisitor < EmitVisitor
    ...
    def visit_Type(type, depth = 0)
      visit(type.value, type.name, depth)
    end

    def visit_Sum(sum, name, depth)
      if simple_sum?(sum)
        simple_sum(sum, name, depth)
      else
  ...
  class PrototypeVisitor < EmitVisitor
    ...
    def visit_Product(prod, name)
      emit_function(name, get_c_type(name),
        get_args(prod.fields), [], union = false)
    end
  end
  ...
  class Obj2ModVisitor < PickleVisitor
    ...
    def visit_Field(field, name, sum: nil, prod: nil, depth: 0)
      ctype = get_c_type(field.type)
      check = if field.opt?
        "exists_not_none(obj, &PyId_%s)" % [field.name]
      else
        "_PyObject_HasAttrId(obj, &PyId_%s)" % [field.name]
      end
  ...
  class MarshalPrototypeVisitor < PickleVisitor
    def prototype(sum, name)
      ctype = get_c_type(name)
      emit("static int marshal_write_#{name}(PyObject **, int *, #{ctype});", 0)
    end
  ...
  class ObjVisitor < PickleVisitor
    ...
    def emit_sequence(field, value, depth, emit)
      ...
      set("value", field, "asdl_seq_GET(seq, i)", depth + 1)
